...
1. **Raw notice to review gate:** the `LIVE · STRANDS` setup view with extracted citations and the correction-review interrupt visible.
2. **Inspectable orchestration:** the Agent Run panel showing intake, plan, review gate, coordination, and the contractor handoff.
...
2. **The workflow wakes itself up:** CloudWatch logs show the private scheduler worker executing a hashed workflow reference and advancing the campaign.
